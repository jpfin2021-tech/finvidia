  ...
  useEffect(() => {
    async function fetchCreatorDirectory() {
      ...
      try {
        ...
        // Direct fetch from channels table
        const { data: channelsData, error: chanError } = await supabase
          .from('channels')
          .select('id, name, handle, avatar_url, slug');
        ...
        // Paginated fetch for all video view stats
        let allVids: any[] = [];
        let page = 0;
        let hasMore = true;
        ...
        for (const v of allVids) {
          ...
          if (!videoStatsMap.has(cid)) {
            videoStatsMap.set(cid, { views, count: 1 });
          } else {
            const stat = videoStatsMap.get(cid)!;
            stat.views += views;
            stat.count += 1;
          }
        }
        ...
        const formatted: CreatorItem[] = channelsData.map((c: any) => {
          const stats = videoStatsMap.get(c.id) || { views: 0, count: 0 };
          return {
            id: c.id,
            name: c.name,
            handle: c.handle,
            ...
            avg_views_per_video: stats.count > 0 ? Math.round(stats.views / stats.count) : 0,
          };
        });
    ...
    <div className="pt-24 pb-20 px-6 md:px-12 max-w-7xl mx-auto min-h-screen bg-[#09090b]">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-400 self-end sm:self-center">
          ...
          >
            <option value="views">Most Total Reaction Views</option>
            <option value="reactions">Most Movies Indexed</option>
            <option value="avg_views">Highest Avg Views per Video</option>
